...
public class InMemoryTaskManager implements TaskManager {
    ...
    private final HashMap<Integer, Epic> epics = new HashMap<>();//Хранит task.Epic.
    ...
    public void clearTasks() { //Удаление всех задач.
        historyManager.removeAll(tasks);
        tasks.clear();
    }

    @Override
    public void clearEpics() { //Удаление всех task.Epic.
        historyManager.removeAll(epics);
        historyManager.removeAll(subtasks);
    ...
    @Override
    public Optional<Task> getTask(int id) { //Получение task.Task по идентификатору.

        if (tasks.containsKey(id)) { //Проверяет если такой ID в задачах.
            historyManager.add(tasks.get(id));
        } else {
            System.out.println("Не существует такого ID task.Task");
        }
        return Optional.ofNullable(tasks.get(id));
    }

    @Override
    public Optional<Epic> getEpic(int id) { //Получение task.Epic по идентификатору.

        if (epics.containsKey(id)) { //Проверяет если такой ID в task.Epic.
            historyManager.add(epics.get(id));
        } else {
            System.out.println("Не существует такого ID task.Epic");
        }
        return Optional.ofNullable(epics.get(id));
    }

    @Override
    public Optional<Subtask> getSubtask(int id) { //Получение task.Subtask по идентификатору.
    ...
    @Override
    public void addTask(Task taskInput) { //Добавляет полученный объект task.Task в соответсвующий HashMap и проверяет, если такой ID уже.
    ...
    @Override
    public void addEpic(Epic taskInput) { //Добавляет полученный объект task.Epic в соответсвующий HashMap и проверяет, если такой ID уже.
        boolean isEpicExist = epics.containsKey(taskInput.getId());//Проверяет на наличие task.Epic.
    ...
    @Override
    public void addSubtask(Subtask taskInput) { //Добавляет полученный объект task.Subtask в соответсвующий HashMap и проверяет, если такой ID уже.
        boolean isSubtaskExist = subtasks.containsKey(taskInput.getId());

        if (isSubtaskExist) {
            System.out.println("Подзадача с таким ID уже создана");
        } else {
            boolean isEpicExist = epics.containsKey(taskInput.getId());//если есть task.Epic, к которому подзадача принадлежит.

            if (isEpicExist) {
                subtasks.put(taskInput.getId(), taskInput);
                epics.get((taskInput).getIdEpic()).
                        addSubtask(taskInput); //Добавляет подзадачу в список определенного task.Epic.
                StatusTask.checkStatus(taskInput.getIdEpic(), epics); //Проверяет статус task.Epic после добавления в него подзадачи.
            } else {
    ...
    @Override
    public void removeTaskById(int id) { //Удаление task.Task по идентификатору.
        historyManager.removeById(id);//удаляет task.Task по ID в истории задач
        tasks.remove(id);
    }

    @Override
    public void removeEpicById(int id) { //Удаление task.Epic по идентификатору и его подзадач.
        historyManager.removeById(id);//удаляет task.Epic по ID в истории задач
        for (Subtask subtask : epics.get(id).getSubtasksArray().values()) {//удаляет подзадачи task.Epic-а по ID в истории задач
